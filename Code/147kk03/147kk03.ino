...
#define lo_max_freq 30000000UL // Максимальная частота опоры, Гц.
#define lo_min_freq 400000UL // Минимальная частота опоры, Гц.
...
struct general_set {
  ...
  unsigned long usb_lo_freq_set = 15992700UL; // Начальная частота опоры USB при первом включении.
  unsigned long lsb_lo_freq_set = 15995100UL; // Начальная частота опоры LSB при первом включении.
  ...
  int temp_cal = 0; //калибровка датчика температуры
} general_setting;
...
#define usb_lo_freq general_setting.usb_lo_freq_set
#define lsb_lo_freq general_setting.lsb_lo_freq_set
  ...
  if (knobdown && !knobup) { //Если кноб нажат
    ...
    if (knobupmillis - knobMillis < 1000 && knobupmillis - knobMillis > 100) { //Если кноб отпущен и был нажат и времени от таймера прошло 100Мс
      if (menu < 5 && menu > 0 && actfmenuf) { //Если 0<меню<5 и крутили енкодер в быстром меню, то выйти на главный экран
        ...
        if (menu > 15) menu = 5; //Если меню больше 16 перейти на меню 5
      }
...
void readencoder() { // работа с енкодером
  ...
  if (newPosition != oldPosition) { // ЕСЛИ КРУТИЛИ энкодер
    ...
    switch (menu) {
      ...
      case 7: //Настройка LO гетеродина LSB
        if (newPosition > oldPosition && lsb_lo_freq <= lo_max_freq) lsb_lo_freq += arraystp[stp];
        if (newPosition < oldPosition && lsb_lo_freq >= lo_min_freq) lsb_lo_freq -= arraystp[stp];
        lsb_lo_freq = constrain(lsb_lo_freq, lo_min_freq, lo_max_freq);
        vfosetup();
        break;

      case 8: //Настройка LO гетеродина USB
        if (newPosition > oldPosition && usb_lo_freq <= lo_max_freq) usb_lo_freq += arraystp[stp];
        if (newPosition < oldPosition && usb_lo_freq >= lo_min_freq) usb_lo_freq -= arraystp[stp];
        usb_lo_freq = constrain(usb_lo_freq, lo_min_freq, lo_max_freq);
        vfosetup();
        break;
...
void mainscreen() { //Процедура рисования главного экрана
  ...
  switch (menu) {
    ...
    case 7: //Меню 7 - Настройка LO гетеродина LSB
      display.setTextSize(2);
      display.println(lsb_lo_freq);
      display.setTextSize(1);
      display.print(menu);
    ...
    case 8: //Меню 8 - Настройка LO гетеродина USB
      display.setTextSize(2);
      display.println(usb_lo_freq);
      display.setTextSize(1);
      display.print(menu);
...
void vfosetup() {
  if (mode) {
    si.set_freq((vfo_freq + usb_lo_freq + RXifshift), 0, (usb_lo_freq + RXifshift));
  }
  else {
    si.set_freq((vfo_freq + lsb_lo_freq + RXifshift), 0, (lsb_lo_freq + RXifshift));
  }
}
