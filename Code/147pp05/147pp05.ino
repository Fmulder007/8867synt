...
#define bfo_max_freq 30000000UL // Максимальная частота второй опоры, Гц.
#define bfo_min_freq 1000000UL // Минимальная частота второй опоры, Гц.
...
#define powerpin 15 // Порт показометра мощности. А1
#define smeterpin 16 // Порт S-метра A2
#define mybattpin 21 // Порт датчика АКБ АDC7
...
struct general_set {
  ...
  unsigned long usb_bfo_freq_set = 10200000UL; // Начальная частота опоры USB при первом включении.
  unsigned long lsb_bfo_freq_set = 11200000UL; // Начальная частота опоры LSB при первом включении.
...
void pushknob () {  // Обработка нажатия на кноб
  ...
  if (knobdown && !knobup) { //Если кноб нажат
    ...
    if (knobupmillis - knobMillis >= 1000) { //Если длительное нажатие
      if (menu == 0) menu = 4; //Перейти в инжменю
    ...
    if (knobupmillis - knobMillis < 1000 && knobupmillis - knobMillis > 100) { //Если кноб отпущен и был нажат и времени от таймера прошло 100Мс
      if (menu < 4 && menu > 0 && actfmenuf) { //Если 0<меню<3 и крутили енкодер в быстром меню, то выйти на главный экран
        actfmenuf = false;
        menu = 0;
      }
      else {
        menu ++; //Переходим на меню дальше
        if (menu == 4) menu = 0; //Если меню 3 выйти на главный экран
        if (menu > 17) menu = 4; //Если меню больше 17 перейти на меню 4
      }
...
void readencoder() { // работа с енкодером
  ...
  if (newPosition != oldPosition && digitalRead(myEncBtn)) { // ЕСЛИ КРУТИЛИ НЕ нажатый энкодер
    ...
    switch (menu) {
      ...
      case 16: // Настройка делителя импульсов энкодера
      ...
      case 17: //Калибровка термодатчика
...
void powermeter () { // Измеритель уровня выхода
  mypower = map(analogRead(powerpin), 0, 1023, 0, 567);
  mysmeter = map(analogRead(smeterpin), 0, 1023, 0, 135);
}
...
void mainscreen() { //Процедура рисования главного экрана
  ...
  switch (menu) {
    ...
    case 0: //Главный экран
      ...
      if (mysmeter > 5) display.fillRect(100, 0, mysmeter / 5, 5, WHITE);
      display.fillRect(100 + mysmeter / 5, 0, 1, mysmeter % 5, WHITE);
      break;
    ...
    case 7: //Меню 6 - Настройка BFO гетеродина LSB
    ...
    case 8: //Меню 7 - Настройка BFO гетеродина USB
      display.setTextSize(2);
    ...
    case 9: //Меню 8 - Настройка опорного гетеродина 500кГц
    ...
    case 10: //Меню 8 - Настройка калибровки кварца
    ...
    case 11: //Меню 9 - Настройка калибровки по питанию
    ...
    case 12: //Меню 10 - Количество диапазонов
    ...
    case 13: //Меню 11 - Настройка Часов
    ...
    case 14: //Меню 12 - Настройка Минут
      if (tm.Minute < 10) display.print("0");
    ...
    case 15: //Меню 12 - Reverse Encoder
      if (reverse_encoder) {
        display.println("Yes");
      }
      else
    ...
    case 16: //Меню 13 - Reverse Encoder
    ...
    case 17: //Меню 13 - Калибровка термодатчика
...
void vfosetup() {
  if (mode) {
    si.set_freq((vfo_freq + usb_bfo_freq + lo_freq + lo_cal_freq), (lo_freq + lo_cal_freq), (usb_bfo_freq));
  }
  else {
    si.set_freq((vfo_freq + lsb_bfo_freq - (lo_freq + lo_cal_freq)), (lo_freq + lo_cal_freq), (lsb_bfo_freq));
  }
}
